...
It is suggested to use the epiphany web browser, since it is relatively fast.
...
Start the chromium web browser (unfortunately epiphany does not work correctly for this task) and go to the \webIOPi header webpage.
...
\item [1.3.] Connect an LED with a 1~kOhm resistor as in checkpoint 1.1 above. Start python interactively to set GPIO 24. 
Using the code templates, import the webiopi framework, make a GPIO object and turn the LED on and off.
%
\begin{verbatim}
...
Write a python script to toggle the LED status every time the push button switch is pushed. \\
