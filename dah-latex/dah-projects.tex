...
The SPI I/O expander chip (MCP23S17) is supported by the \webiopi framework. In checkpoint 4 you used an 8 bit I2C expander chip in a very similar way, so you can build upon this work. For this project we recommend that you use the MCP23S17 chip, which is a 16 bit SPI expander. While the
...
Download the files ups-15.bin, ups-15-small.bin and ups-15-MC.bin   from the DAH Dropbox, 
...
\item The results so far probably look "Äùquite good"Äù by eye. i.e. the signal shape plotted on
...
the "Äùresiduals"Ä. A residual is the difference between the data in the binned histogram
